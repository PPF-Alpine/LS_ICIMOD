...
threats_long <- threats_stresses_species_broad |>
  ...
  distinct() |>
  group_by(sciname,threats_broad) |> # for unique species and unique threats
  slice(1) |>
  ungroup()
...
ggsave(filename = paste0(plot_output_path, "IUCN_threats_HKH_mammals.jpg"),
       plot = plot_count, width = 9, height = 5, dpi = 300)
